...
from tools.ci_ceiling_manifest import HARDWARE_WORKFLOW_TO_GATES, WORKFLOW_TO_GATES, build, build_hardware
...
class CiCeilingManifestTests(unittest.TestCase):
    ...
    def make_run(self, name, *, run_number, status="completed", conclusion="success", event="push"):
        return {
            "id": run_number, "name": name, "head_sha": self.SHA, "event": event,
            "run_number": run_number, "created_at": f"2026-09-23T13:{run_number % 60:02d}:00Z",
            "status": status, "conclusion": conclusion, "html_url": f"https://example.invalid/{run_number}",
        }

    def test_complete_success_set_passes_exact_software_policy(self):
        runs = [self.make_run(name, run_number=i+1) for i, name in enumerate(WORKFLOW_TO_GATES)]
    ...
    def test_hardware_manifest_is_separate(self):
        runs = [self.make_run(name, run_number=i+1) for i, name in enumerate(HARDWARE_WORKFLOW_TO_GATES)]
        ...
        self.assertEqual(evaluate_hardware(complete)["status"], "HARDWARE_BOUNDARY_PASS")

    def test_missing_or_in_progress_is_not_ready(self):
        names = list(WORKFLOW_TO_GATES)
        runs = [self.make_run(name, run_number=i+1) for i, name in enumerate(names[:-1])]
        manifest, details = build({"workflow_runs": runs}, self.SHA)
        self.assertFalse(details["ready"])
        self.assertNotEqual(evaluate(manifest)["status"], "SOFTWARE_CEILING_PASS")
        runs.append(self.make_run(names[-1], run_number=99, status="in_progress", conclusion=None))
        manifest, details = build({"workflow_runs": runs}, self.SHA)
        self.assertFalse(details["ready"])
    ...
    def test_pull_request_run_cannot_substitute_for_push_evidence(self):
        names = list(WORKFLOW_TO_GATES)
        runs = [self.make_run(name, run_number=i+1) for i, name in enumerate(names)]
        target = names[0]
